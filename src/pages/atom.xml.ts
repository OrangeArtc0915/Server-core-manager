...
<feed xmlns="http://www.w3.org/2005/Atom">
  ...
  <language>${siteConfig.lang}</language>`;
	...
	for (const post of posts) {
		...
		for (const img of images) {
			...
			if (
				...
				if (imageMod) {
					...
					// url() 补上 base，否则订阅里图片会指向域名根目录
					img.setAttribute("src", new URL(url(optimizedImg.src), context.site).href);
				} else {
		...
		// 添加Atom条目
		const postUrl = new URL(url(`/posts/${post.slug}/`), context.site).href;
